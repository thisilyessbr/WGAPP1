...
import { ExtractedCommerceParams, EcommerceIntentParser } from '../ecommerce/EcommerceIntent';
...
export interface PolicySignals {
  isPolicy: boolean;
  intent: 'RETURNS' | 'CARE' | 'SHIPPING' | 'TRACKING' | 'WARRANTY' | 'PAYMENT' | 'STORE_INFO' | 'KNOWLEDGE_RETRIEVAL';
...
export class TurnDecisionResolver {
  ...
  private static readonly SHIPPING_TERMS = /(?:shipping|delivery|deliver|deliveries|delivered|livraison|livrer|livrez|livré|expédition|expédier|envoi|envoyer|توصيل|التوصيل|شحن|الشحن|مصاريف\s+الشحن|ثمن\s+التوصيل|سعر\s+التوصيل|وقت\s+التوصيل|مدة\s+التوصيل|توصل|توصلو|كيوصل|كتوصل|يوصل|يوصلو|توصلني|يوصلني|tewsil|tawsil|tawseel|twsil|ywsl|twsl|kiwsl|kitwsl|ywsal|twsal)/iu;
  ...
  private static readonly STORE_INFO_TERMS = /(?:hours|opening\s+hours|business\s+hours|horaires|heures\s+d['’]ouverture|store\s+location|locations|branch|branches|boutique|magasin|magasins|أوقات\s+العمل|ساعات\s+العمل|مواعيد|فرع|فروع|محل|محلات|موقعكم|فين\s+كاينين|عنوان|العنوان|frou3|fara3|politique|guide)/iu;
  ...
  public static detectPolicySignals(text: string): PolicySignals {
    ...
    const isPolicy = isReturns || isCare || isShipping || isTracking || isWarranty || isPayment || isStoreInfo;
  ...
  public static resolve(input: TurnDecisionInput): TurnDecision {
    ...
    // 6. Parse ecommerce params if not supplied
    const ecomParams = input.ecommerceParams !== undefined
      ? input.ecommerceParams
      : EcommerceIntentParser.parse(text, input.productContext, language);
    ...
    if (explicitProductName && (EcommerceIntentParser.isNonProductReference(explicitProductName) || explicitProductName.length <= 2)) {
    ...
    if (!explicitProductName && isPolicyQuery) {
      ...
      if (prodMatch && prodMatch[1]) {
        ...
        if (!EcommerceIntentParser.isNonProductReference(candidate) && candidate.length > 2) {
          explicitProductName = EcommerceIntentParser.cleanProductName(candidate);
        }
      }
    }
    ...
    const hasProductMention = Boolean(
      explicitProductName ||
      (ecomParams?.sku && !EcommerceIntentParser.isNonProductReference(ecomParams.sku)) ||
      (isAnaphoricPolicyQuery && input.productContext?.selectedProductId)
    );

    const resolvedContext = this.resolveProductContext(ecomParams, input.productContext, isPolicyQuery, hasProductMention);
    ...
    if (
      ...
      (ecomParams && ['PRODUCT_SEARCH', 'PRODUCT_DETAIL', 'PRICE', 'AVAILABILITY', 'VARIANT_SELECTION', 'COMPARE', 'RECOMMENDATION'].includes(ecomParams.intent))
    ) {
      return {
        domain: 'ECOMMERCE',
  ...
  public static resolveProductContext(
    ...
    const isExplicit = Boolean(
      ...
      extracted?.productName ||
      extracted?.category ||
      (extracted?.intent === 'PRODUCT_SEARCH') ||
